# This file contains function(s) that translates raw bytes into assembly
# instructions using x86opcodesTable and some python logic.
    ...
    for i in bytes:
        ...
        if times == 0:
            ...
            if i in x86opT.x86opcodes:
                ...
                # need to add to this after_instruction every time this
                # variable (after_instruction) is usesd inside an if
                intruction_len_for_check = 51 + len(instruction)
                should_print = True
                ...
                if (i == "06" or i == "07" or i == "0E" or i == "16" or i == "17" or i == "1E" or i == "1F" or i == "27" or i == "2F" or i == "40" or i == "41" or i == "42" or i == "43"
                    ...
                    check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "   " + f"{bcolors.FAIL}" + to_display + \
                        after_byte + f"{bcolors.WARNING}" + instruction + after_instruction + f"{bcolors.RESET}"
                    ...
                    try:
                        try:
                            check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "  " + f"{bcolors.FAIL}" + to_display + after_byte + f"{bcolors.OKGREEN}" + instruction.split(
                                " ")[0] + " " + f"{bcolors.WARNING}" + instruction.split(" ")[1] + " " + instruction.split(" ")[2] + after_instruction + f"{bcolors.RESET}"
                        except BaseException:
                            check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "  " + f"{bcolors.FAIL}" + to_display + after_byte + f"{bcolors.OKGREEN}" + instruction.split(
                                " ")[0] + " " + f"{bcolors.WARNING}" + instruction.split(" ")[1] + after_instruction + f"{bcolors.RESET}"
                    except BaseException:
                        check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "  " + f"{bcolors.FAIL}" + to_display + after_byte + \
                            f"{bcolors.OKGREEN}" + instruction.split(" ")[0] + f"{bcolors.WARNING}" + after_instruction + f"{bcolors.RESET}"
                    print(check1)
                    after_byte = ""
                    ...
                    check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "   " + f"{bcolors.FAIL}" + to_display + \
                        after_byte + f"{bcolors.WARNING}" + instruction + after_instruction + f"{bcolors.RESET}"
                    intruction_len_for_check = 51 + \
                        len(instruction) + len(after_instruction)
                    if len(check1) < intruction_len_for_check:
                        for _ in range(intruction_len_for_check - len(check1)):
                            after_byte += " "
                    if isClean:
                    ...
                    if isClean:
                        after_instruction = after_instruction + " " * \
                            (32 - lenWholeOpcode) + "; " + str(int(TEMPvar, 16))
                    else:
                        after_instruction = after_instruction + " " * \
                            (32 - lenWholeOpcode) + f"{bcolors.OKGREEN}; " + str(int(TEMPvar, 16))
                    check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "   " + f"{bcolors.FAIL}" + to_display + \
                        after_byte + f"{bcolors.WARNING}" + instruction + after_instruction + f"{bcolors.RESET}"
                    intruction_len_for_check = 51 + \
                        len(instruction) + len(after_instruction)
                    ...
                    after_byte = " " + bytes[counter1 + 1] + " " + bytes[counter1 +
                                                                         2] + " " + bytes[counter1 + 3] + " " + bytes[counter1 + 4]
                    TEMPoffset = " 0x" + \
                        bytes[counter1 + 4] + bytes[counter1 + 3] + bytes[counter1 + 2] + bytes[counter1 + 1]
                    ...
                    if TEMPoffset_to_dict in ascii_dict:
                        ...
                        lenWholeOpcode = len(instruction) + \
                            len(after_instruction)
                        if isClean:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + "; str: " + str(ascii_dict[TEMPoffset_to_dict])
                        else:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + f"{bcolors.OKGREEN}; str: {bcolors.WARNING}" + str(ascii_dict[TEMPoffset_to_dict])
                        ...
                        if isClean:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + "; str: " + str(ascii_dict[TEMPoffset_to_dict2])
                        else:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + f"{bcolors.OKGREEN}; str: {bcolors.WARNING}" + str(ascii_dict[TEMPoffset_to_dict2])
                    else:
                        after_instruction = TEMPoffset
                    check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "   " + f"{bcolors.FAIL}" + to_display + \
                        after_byte + f"{bcolors.WARNING}" + instruction + after_instruction + f"{bcolors.RESET}"
                    ...
                    if int(bytes[counter1 + 1], 16) >= 128:
                        if int(bytes[counter1 + 1], 16) == 255:
                            ...
                            after_instruction = " " + \
                                hex(offset1 - (256 - (int(bytes[counter1 + 1], 16) + 2)))
                    else:
                        after_instruction = " " + \
                            hex(offset1 + (int(bytes[counter1 + 1], 16) + 2))
                    check1 = f"{bcolors.OKBLUE}" + str(hex(offset1)) + "   " + f"{bcolors.FAIL}" + to_display + \
                        after_byte + f"{bcolors.WARNING}" + instruction + after_instruction + f"{bcolors.RESET}"
                    intruction_len_for_check = 51 + \
                        len(instruction) + len(after_instruction)
                    ...
                    after_byte = " " + bytes[counter1 + 1] + " " + bytes[counter1 + 2] + " " + \
                        bytes[counter1 + 3] + " " + bytes[counter1 + 4] + " " + bytes[counter1 + 5]
                    _81var = ""
                    if bytes[counter1 + 1] == "C3":
                        ...
                        if isClean:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + "; " + str(int(_81var, 16))
                        else:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + f"{bcolors.OKGREEN}; " + str(int(_81var, 16))
                    ...
                    after_byte = " " + \
                        bytes[counter1 + 1] + " " + bytes[counter1 + 2]
                    ...
                    if bytes[counter1 + 1] == "C2":
                        after_instruction = " edx, " + \
                            "0x" + str(TEMPvar.lower())
                        lenWholeOpcode = len(instruction) + \
                            len(after_instruction)
                        if isClean:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + "; " + TEMPvar4
                        else:
                            after_instruction = after_instruction + " " * \
                                (32 - lenWholeOpcode) + f"{bcolors.OKGREEN}; " + TEMPvar4
                        ...
                        after_instruction = " dword [var_4h], " + \
                            "0x" + str(TEMPvar3.lower())
                        lenWholeOpcode = len(instruction) + \
                            len(after_instruction)
                        if isClean:
                            after_instruction = after_instruction + " " * \
                        ...
                        TEMPvar4 = str(int(bytes[counter1 + 3], 16))
                        instruction = "CMP"
                        after_instruction = " dword [arg_8h], " + \
                            "0x" + str(TEMPvar3.lower())
                        lenWholeOpcode = len(instruction) + \
                            len(after_instruction)
                        if isClean:
                    ...
                    if bytes[counter1 + 1] == "1C":
                        ...
                        after_instruction = " eax, dword [ebx + " + \
                            str(TEMPvar2) + "]"
                        TEMPvar = 2
                    else:
                        should_print = False
